...
void lcd_slope(uint8_t temperature, degree reading, double battery, bool USART_enable)
{
    ...
    gfx_puts("BATERIA:");
    gfx_setCursor(90, 100);
    sprintf(buf, "%3.2f", battery);
    gfx_puts(buf);
    gfx_setCursor(130, 100);
    gfx_puts("V");
...
void console_puts_all(data xyzData, uint8_t temperature, double battery)
{
    ...
    console_puts(buf);
    console_puts("\t");
    snprintf(buf, sizeof(buf), "%3.2f", battery);
    console_puts(buf);
    console_puts("\n");
}
