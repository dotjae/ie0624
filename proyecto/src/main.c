...
static void send_usb_data(usbd_device *usbd_dev, const char *data)
{   
    usbd_ep_write_packet(usbd_dev, 0x82, data, strlen(data));
}

static usbd_device* cdcam_init(void)
{
...
int main(void)
{
    ...
    usbd_device *usbd_dev = cdcam_init();
	...
	while (1) {
		...
		usbd_poll(usbd_dev);
        send_usb_data(usbd_dev, "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaa");
    }
